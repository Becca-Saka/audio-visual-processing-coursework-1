...
def train_model():
    ...
    for audio_file in sorted(glob.glob('names_audio_wav/*.wav')):
        ...
        if 10 < label_number < 20:
            feats = feature_extraction(r_in, fs_in, has_noise=True)
        else:
...
def librosa_features(audio, fs_in, n_mfcc=32):
    # Convert stereo to mono if needed
    if audio.ndim == 2:
        audio = audio.mean(axis=1)
    # Extract MFCCs
    mfcc = librosa.feature.mfcc(y=audio, sr=fs_in, n_mfcc=n_mfcc)
    # Take mean and std across time
    mfcc_mean = np.mean(mfcc, axis=1)
    mfcc_std = np.std(mfcc, axis=1)
    return np.concatenate([mfcc_mean, mfcc_std])

def test_librosa_model(audio_file, model, LE):
    r_in, fs_in = sf.read(audio_file, dtype='float32')
    feats = librosa_features(r_in, fs_in)
...
def train_with_librosa():
    data, labels = [], []
    for audio_file in sorted(glob.glob('names_audio_wav/*.wav')):
        r_in, fs_in = sf.read(audio_file, dtype='float32')
        feats = librosa_features(r_in, fs_in)
        label = ''.join([c for c in audio_file.split('/')[-1].split('.')[0] if not c.isdigit()])
        data.append(feats)
        labels.append(label)
    data = np.array(data)
    labels, LE = labelEncoder(labels)
...
model, LE, history = train_model()  # Train once and get model + encoder
lib_model, lib_LE, lib_history = train_with_librosa()
# r_in, fs_in = SoundClass().record_ns(fs=44100,seconds= 3)
# test_model(r_in, fs_in, model, LE)
# test_audio_path = 'tester/Emma.wav'
# for i in range(5):
#     r_in, fs_in = sf.read(test_audio_path, dtype='float32')
#     print(f"Testing with audio, {test_audio_path} and sample rate {fs_in}")
#     if r_in.ndim == 2:
#         r_in = r_in.mean(axis=1)
#     predicted_name, confidence = test_model(r_in, fs_in, model, LE)  # Pass model and LE
# test_audio_path = 'names_audio_wav/Ahmed004.wav'
results_training = []
matches_training = 0
total_training = 0
for audio_file in sorted(glob.glob('names_audio_wav/*.wav')):
    ...
    custom_pred, _ = test_model(r_in, fs_in, model, LE)
    librosa_pred, _ = test_librosa_model(audio_file, lib_model, lib_LE)
    total_training += 1
    if custom_pred == librosa_pred:
        matches_training += 1
    ...
    results_training.append({
            ...
            'Predicted Name': custom_pred.strip(),
        })
...
print(f"Agreement between custom vs librosa: {matches}/{total} ({matches / total * 100:.2f}%)")
print(f"Agreement between custom vs librosa training: {matches_training}/{total_training} ({matches_training / total_training * 100:.2f}%)")
plt.figure(figsize=(10, 6))
epochs_custom = range(1, len(history.history['accuracy']) + 1)
epochs_librosa = range(1, len(lib_history.history['accuracy']) + 1)

# Custom features accuracy
plt.plot(epochs_custom, history.history['accuracy'], 'b-', label='Custom Train')
plt.plot(epochs_custom, history.history['val_accuracy'], 'b--', label='Custom Val')

# Librosa MFCC accuracy
plt.plot(epochs_librosa, lib_history.history['accuracy'], 'r-', label='Librosa Train')
plt.plot(epochs_librosa, lib_history.history['val_accuracy'], 'r--', label='Librosa Val')
